...
from my_axis_camera.cfg import PTZConfig
...
class StateThread(threading.Thread):
	...
	def run(self):
		# jared note: it appears that the maximum throughout of this loop (which 
		# depends on querying via an http connection) and publishing in ros is 
		# ~44 updates/second
		# That said, there are duplicates states when the camera is moving (meaning
		# that while we can reach out to the camera for updates 44x/second, the 
		# camera itself does not update its own state that frequently.  
		# What we really want is to limit the update rate to the camera's internal
		# update rate.  Unfortunately, I have not been able to find out what that is.
		# My experimental testing shows aliasing happening at 22x/second.  At half that,
		# (11x/second), I have not seen it.  To give myself a little buffer, I reduce it
		# to 8x/second.
		r = rospy.Rate(8.0)
		self.msg = Axis()
...
class AxisPTZ:
	...
	def __init__(self, hostname, username, password, flip, speed_control):
		...
		self.sub_mirror = rospy.Subscriber("mirror", Bool, self.mirrorCallback,queue_size=1)
	...
	def createCmdString(self):
		...
		if self.speedControl:
			self.cmdString += 'continuouspantiltmove=%d,%d&' % (int(self.msg.pan), int(self.msg.tilt)) \
							...
							+ 'continuousbrightnessmove=%d&' % (int(self.msg.brightness))
			# Note that brightness adjustment has no effect for Axis 214PTZ.
			if self.msg.autofocus: self.cmdString += 'autofocus=on&'
			else:				   self.cmdString += 'autofocus=off&continuousfocusmove=%d&' % (int(self.msg.focus))
			self.cmdString += 'autoiris=on'
		else:
			# set params based on command type
			if self.msg.cmd_type == 'ptz':
				self.cmdString += 'pan=%f&tilt=%f&' % (self.msg.pan, self.msg.tilt)\
								+ 'zoom=%f&' % (int(self.msg.zoom)) \
								+ 'brightness=%f&' % (int(self.msg.brightness))
			elif self.msg.cmd_type == 'pt':
				self.cmdString += 'pan=%f&tilt=%f&' % (self.msg.pan, self.msg.tilt)\
								+ 'brightness=%f&' % (int(self.msg.brightness))
			elif self.msg.cmd_type == 'focus':
				self.cmdString += 'focus=%f&' % (int(self.msg.focus)) 
			elif self.msg.cmd_type == 'areazoom':		
				self.cmdString += 'areazoom=%d,%d,%d' % (self.msg.x, self.msg.y, self.msg.zoom_factor)
			elif self.msg.cmd_type == 'center':
				self.cmdString += 'center=%d,%d' % (self.msg.x, self.msg.y)

			# set autofocus
			if self.msg.autofocus:
				self.cmdString += 'autofocus=on&'
			else:
				self.cmdString += 'autofocus=off&focus=%d&' % (int(self.msg.focus))
