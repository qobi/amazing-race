...
std::unique_ptr<nav_msgs::OccupancyGrid> CreateOccupancyGridMsg(
  ...
  for (int y = height - 1; y >= 0; --y) {
	...
	// jared note: this is the original code...which assigns cells a value of -1 if there are zero observations
	// of that cell.  (This can be confusing for downstream nodes...because the coloring scheme in rviz is 
	// different.  So, the coloring scheme indicates there is SOME data for the cell (i.e. a value between 0-100
	// ...when in fact there is none and the cell value is assigned -1.
	// Nevermind this for now.  I may be wrong about this.
	// I change that below to have the cell value reflect the coloring in rviz   
      int value =
          observed == 0
              ? -1
              : ::cartographer::common::RoundToInt((1. - color / 255.) * 100.);
     // int value = ::cartographer::common::RoundToInt((1. - color / 255.) * 100.);
	 // if (value == 50) {
	 //     value = -1;
	 // }

	 // for downstream nodes, set the 'on the fence' pixels to unknown
	 // if (value > 35 && value < 60) {
	 //     value = -1;
	 // }
	  // for downstream nodes, set any "i'm getting more confident this is an obstacle" cells to obstacles 
	  if (value > 60) {
	      value = 100;
	  }
      CHECK_LE(-1, value);
